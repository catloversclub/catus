...
export class AuthController {
  ...
  async kakaoCallback(
    ...
    const redirectBase =
      this.config.get<string>("KAKAO_REDIRECT_URL") || "https://joodang.com/auth/kakao"

    let redirectUrl = redirectBase
    if (stateToken) {
      const secret = this.config.get<string>("KAKAO_CLIENT_SECRET") || "dev-secret"
      const payload = decryptState<{ returnTo?: string }>(stateToken, secret)
      if (payload?.returnTo) {
        redirectUrl += safeReturnTo(payload.returnTo, redirectBase.replace(/\/$/, "")) ?? ""
      }
    }
